  ...
  async function load() {
    ...
    try {
      ...
      for (const mesh of result.meshes) {
        ...
        // metalness 高时无环境贴图的面会发黑；side:DoubleSide 是"整块发黑"的解药 ——
        // OCCT 剖分出的网格常有局部法线反向，单面材质从外看那些面就是黑的
      ...
      scene.add(group);
      // 光照：three r155 起默认物理光照单位，旧的 ambient 0.75 + 单向平行光在新单位下
      // 明显偏暗，且背面只有环境光 —— 线上反馈"背面的管脚都看不清"。
      // 改为半球光打底（上下异色模拟天空/地面反射）+ 主光 + 背面补光，三面受光。
