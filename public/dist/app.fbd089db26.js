...
function Step3DViewer({
  ...
  async function load() {
    ...
    try {
      ...
      const [occt, THREE, orbitMod] = await Promise.all([loadOcct(setMsg), dynImport(THREE_CDN), dynImport(ORBIT_CDN)]);
  ...
  }, "\u5DF2\u6E32\u67D3 ", meshCount, " \u4E2A\u7F51\u683C \xB7 \u5DE6\u952E\u65CB\u8F6C / \u6EDA\u8F6E\u7F29\u653E / \u53F3\u952E\u5E73\u79FB"), /*#__PURE__*/React.createElement("div", {
    ref: hostRef,
    style: {
      borderRadius: 8,
      overflow: "hidden",
...
function GraphicsPanel({
  ...
  const tryAiPinout = async () => {
    ...
    try {
      ...
      setPdfWarn("AI 未能给出可信的引脚定义（引脚数不符或自评不确定），已保持仅编号符号");
      setAiTry(true);
    } catch (err) {
...
function Workbench({
  ...
  const [procurement, setProcurement] = useState({
    ...
    currency: "USD",
    inStockOnly: true
  }); // default | scenario | user_override
  ...
  const onDrop = (e, i) => {
    ...
    setReorderNote("已按你手动调整的顺序提交；场景切换不会覆盖它");
  };
  ...
  }, "\uD83C\uDFAF \u5E94\u7528\u9886\u57DF"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: C.textMute,
      marginBottom: 10
    }
  }, "\u4E0D\u540C\u5E94\u7528\u4E0B\u6700\u4F18\u66FF\u4EE3\u6599\u4E0D\u540C\uFF0C\u5C06\u8C03\u6574\u53C2\u6570\u6743\u91CD\u4E0E\u786C\u7EA6\u675F"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
      gridTemplateColumns: "repeat(3,1fr)",
      gap: 6
    }
  }, APPLICATIONS.map(a => /*#__PURE__*/React.createElement("button", {
    key: a.code,
    "aria-pressed": application === a.code,
    onClick: () => {
      setApplication(a.code);
      if (orderSource === "user_override") {
        setReorderNote("已保留你手动调整的顺序；如需应用场景排序请点「恢复场景默认排序」");
        return;
      }
      const next = reorderByApplication(params, a.code);
      const changed = next.some((p, i) => p.id !== params[i]?.id);
      if (changed) {
        setParams(next);
        setOrderSource("scenario");
        const moved = next[0]?.name;
        const hard = (APP_HARD[a.code] || []).filter(h => next.some(p => `${p.name}`.includes(h)));
        setReorderNote(`已重排：「${moved}」提升至第 1 位` + (hard.length ? `；建议将 ${hard.join("、")} 设为硬约束` : ""));
      } else {
        setOrderSource(a.code === "generic" ? "default" : "scenario");
        setReorderNote("该场景下参数顺序与当前一致，未做调整");
      }
    },
    title: a.desc,
    style: {
      ...
      background: application === a.code ? C.greenLight : "#fff",
      cursor: "pointer",
      textAlign: "center"
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, a.label)))), reorderNote && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8,
      fontSize: 11,
      color: C.indigo,
  ...
  }, "\uD83D\uDCA1 ", reorderNote, orderSource === "user_override" && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      const next = reorderByApplication(params, application);
      setParams(next);
      setOrderSource("scenario");
      setReorderNote(`已恢复「${APPLICATIONS.find(a => a.code === application)?.label}」默认排序`);
    },
    style: {
      marginLeft: 8,
      padding: "2px 8px",
  ...
  }, "\u6062\u590D\u573A\u666F\u9ED8\u8BA4\u6392\u5E8F"))), mode === "lowCost" && /*#__PURE__*/React.createElement("div", {
    style: {
      borderRadius: 12,
...
function App() {
  ...
  }, "AltPart Pro ", /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      fontWeight: 400,
      color: C.textMute
  ...
  }, "AltPart Pro v", APP_VERSION, " \xB7 ezPLM\u5143\u5668\u4EF6\u5E93 \xB7 \u5B9E\u65F6\u884C\u60C5 \xB7 \u573A\u666F\u5316\u66FF\u4EE3 \xB7 \u786E\u5B9A\u6027\u8BC4\u5206"));
}
