...
- 候选料补齐 DigiKey/Mouser 精确型号查询，不再从 TODO 直接退回 AI 参数。
- `procurement` 全链路生效：地区、数量、包装、币种、现货条件进入召回缓存、报价、门槛和排序。
- 阶梯价改为“采购数量不小于门槛的最大阶梯”，同时处理 MOQ、包装与库存数量。
- `weightsFor()` 正式接入评分引擎，不再只有配置和单测、生产排序却不生效。
- 新增 `PinMapComparison`；只有权威结构化证据的引脚编号、功能和电气类型/方向逐针一致，才设置 `pinVerified=true`。
- Pin-to-Pin 封装比较加入本体尺寸、pitch、EP 与高度，阻止同家族不同几何误判。
- `/analyze` 可返回 HMAC 签名上下文；`/recommend` 不再信任客户端回传的 `original` 参数。
- 生命周期/合规无权威来源时统一返回 Unknown，不再按型号前缀推断或默认 RoHS/REACH=true。
- PDF 和资源代理逐跳校验重定向、DNS/私网、类型和字节上限，降低 SSRF/内存风险。
- 高成本 API 加入可选鉴权和成本型限流；反馈 GET 改为管理员专用。
- BOM 修复新推荐等级契约，改为并发度 2 的有界执行并区分正式结果与待核验结果。
- 新增 v7.0 行为回归测试；生产密钥 live canary 仍需在部署环境执行。
